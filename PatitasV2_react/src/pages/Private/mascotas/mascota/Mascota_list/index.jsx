...
const headers = [
  ...
  { name: "Especie", field: "especie" },
  { name: "Raza", field: "raza" },
  { name: "Disponibilidad", field: "disponibilidad" },
];

function Mascota_list() {
  const store = useContext(StoreContext);
  const [docForPage, setDocForPage] = useState(10);
  const [page, setPage] = useState(1);
  const [maxPage, setMaxPage] = useState(1);
  const [sort, setSort] = useState({ createdAt: -1 }); // ← orden por defecto
  const [total, setTotal] = useState(0);
  const [list, setList] = useState([]);
  const [find, setFind] = useState({});
  const [permissions, set_permissions] = useState({});
  const select = [];
  const populate = ["last_user"];
  ...
  useEffect(() => {
    var temp = {};
    temp.read = store.checkPermissions(["read_mascota"]);
    temp.update = store.checkPermissions(["update_mascota"]);
    temp.delete = store.checkPermissions(["delete_mascota"]);
      ...
      <div className="px-2  py-1 sm:py-2 xl:py-4 ">
        <div className="bg-gray-50 px-1  py-0.5 sm:py-0.5 2xl:py-1  sm:flex flex flex-wrap-reverse sm:items-center w-full">
          ...
          <div className="ml-6 sm:ml-0 flex-none">
            {store.checkPermissions(["create_mascota"]) && (
              <Link
                to="../mascota/create"
                type="button"
